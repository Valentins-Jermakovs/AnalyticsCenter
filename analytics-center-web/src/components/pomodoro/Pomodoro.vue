    ...
    <div class="w-full flex-1 flex flex-col p-5">
        ...
        <div class="flex justify-center">
           ...
           text-5xl sm:text-6xl font-bold">
                ...
                <font-awesome-icon v-if="timerObj.status === 'break'" icon="fa-solid fa-coffee" class="text-2xl" />
                <font-awesome-icon v-else-if="timerObj.status === 'work'" icon="fa-solid fa-laptop" class="text-2xl" />
            </div>
        </div>
        <!-- Sessions indicators -->
        <div class="flex justify-center items-center gap-3 p-5">
            <div v-for="index in sessionIndicators" :key="index" class="w-3 h-3 rounded-full border-2 border-neutral"
                :class="{ 'bg-neutral border-neutral': index === timerObj.currentSession }">
            </div>
        </div>
        <!-- Buttons -->
        <div class="flex justify-center gap-2 flex-wrap">
            <button v-for="btn in controls" :key="btn.key" class="btn btn-neutral" :class="btn.size"
                @click="btn.action">
